...
public final class DeityLandProtectionTierSystem {
    ...
    public static boolean tryUpgradeClaimTier(DeityLandProtectionPlugin plugin, ClaimStore claims, World world, Claim claim, ItemContainer container, DeityLandProtectionUpkeepState state) {
        ...
        if (ownerRef != null) {
            ...
            String msg = lang == DeityLandProtectionLangPreferenceManager.Language.ES ? "Tier de Deity mejorado a " + nextTier + " (radio " + nextRadius + ", esencia/h " + upkeepCost + ")" : "Deity upgraded to Tier " + nextTier + " (radius " + nextRadius + ", essence/hr " + upkeepCost + ")";
    ...
    private static void sendNoUnclaimedTerritoryMessage(DeityLandProtectionPlugin plugin, World world, Claim claim) {
        ...
        DeityLandProtectionLangPreferenceManager.Language lang = plugin.getEffectiveLanguage(ownerRef);
        String msg = lang == DeityLandProtectionLangPreferenceManager.Language.ES ? "No hay territorio sin reclamar para expandir" : "No unclaimed territory to expand into";
        plugin.sendPlayerMessageImmediate(ownerRef, msg);
    }
