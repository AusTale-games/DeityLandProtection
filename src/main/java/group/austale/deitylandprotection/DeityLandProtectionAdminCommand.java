...
public final class DeityLandProtectionAdminCommand
extends AbstractPlayerCommand {
    private final DeityLandProtectionPlugin plugin;

    public DeityLandProtectionAdminCommand(DeityLandProtectionPlugin plugin) {
    ...
    protected void execute(@Nonnull CommandContext context, @Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref, @Nonnull PlayerRef playerRef, @Nonnull World world) {
        ...
        if (actor == null || !this.plugin.isOpBypass(actor)) {
            context.sendMessage(Message.raw((String)"No permission"));
            return;
        }
