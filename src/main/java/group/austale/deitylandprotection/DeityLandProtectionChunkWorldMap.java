...
implements IWorldMap {
    ...
    private static int readPaletteIndex(MapImage image, int pixelIndex) {
        int bpp = image.bitsPerIndex & 0xFF;
        if (bpp <= 0) {
            bpp = 8;
        }
        int startBit = pixelIndex * bpp;
        byte[] packed = image.packedIndices;
        if (packed == null) {
            return 0;
        }
        int result = 0;
    ...
    private static int quantizePackedPixel(int packed) {
        ...
        int gq = g / 32;
        int bq = b / 64;
        return (rq << 24) | (gq << 16) | (bq << 8) | 0xFF;
    }

    private static MapImage rebuildMapImageFromPackedPixels(int width, int height, int[] pixels) {
        ...
        for (int c : pixels) {
            colorToIndex.putIfAbsent(c, colorToIndex.size());
        }
        int paletteSize = colorToIndex.size();
        int bpp = paletteSize <= 1 ? 1 : 32 - Integer.numberOfLeadingZeros(paletteSize - 1);
        if (bpp > 8) {
            bpp = 8;
        }
        int[] palette = new int[paletteSize];
        for (Map.Entry<Integer, Integer> e : colorToIndex.entrySet()) {
            palette[e.getValue()] = e.getKey();
        }
        int numPixels = width * height;
        int totalBits = numPixels * bpp;
        int numBytes = (totalBits + 7) >>> 3;
        byte[] packedOut = new byte[numBytes];
        for (int px = 0; px < numPixels; ++px) {
            int idx = colorToIndex.get(pixels[px]);
            int startBit = px * bpp;
            for (int i = 0; i < bpp; ++i) {
                if ((idx & (1 << i)) == 0) continue;
                int absBit = startBit + i;
                packedOut[absBit >>> 3] = (byte)(packedOut[absBit >>> 3] | (byte)(1 << (absBit & 7)));
            }
        }
        MapImage out = new MapImage();
        out.width = width;
        out.height = height;
        out.palette = palette;
        out.bitsPerIndex = (byte)bpp;
        out.packedIndices = packedOut;
        return out;
    }
